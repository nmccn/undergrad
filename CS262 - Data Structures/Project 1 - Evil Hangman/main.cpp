...
int main() {
    ...
    while (true) {
        //Get the number of Guesses, asking again on bad user inputs.
        int num_guesses = get_integer("How many guesses would you like?");
        cout << endl;

        //Get the desired word length, while meanwhile making sure it is a valid size.
        //initialize the word_length to a non existent key value.
        int word_length = -1;
        while(game.viableLengths[word_length].empty()){
            word_length = get_integer("How long of a word would you like?");
            cout << endl;
        }

        //Ask whether the user wants to know the number of possible words.
        bool show_word_count = get_yesno("Show running total of words? (y/n)");
        cout << endl;

        // prompted for above
        game.start_new_game(num_guesses, word_length, show_word_count);
