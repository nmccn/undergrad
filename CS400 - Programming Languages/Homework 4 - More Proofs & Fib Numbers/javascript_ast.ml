...
(*Doesn't print the (6), or enclosing parenthesis, but I am not really sure we are supposed to print ??*)
let task2_program = ExprProgram(NoPos, ValExpr(NoPos, ClosureVal(StringMap.empty, (Some("dec"), [("x", None)], ReturnBlock(NoPos, 
  IfExpr(NoPos,
      BopExpr(NoPos, VarExpr(NoPos, "x"), GtBop, ValExpr(NoPos, NumVal(0.0))),
      BopExpr(NoPos, VarExpr(NoPos, "x"), PlusBop, CallExpr(NoPos, VarExpr(NoPos,"dec"), [BopExpr(NoPos, VarExpr(NoPos, "x"), MinusBop, ValExpr(NoPos,NumVal(1.0)))])), 
      ValExpr(NoPos, NumVal(0.0)) )),
      None)))) 
let _ = Printf.printf "Task 2:\n%s\n\n" (str_program task2_program)

let task4_program = ExprProgram(NoPos, ValExpr(NoPos, ClosureVal(StringMap.empty, (Some("fib"), [("x", None)], ReturnBlock(NoPos, 
IfExpr(NoPos,
    BopExpr(NoPos, VarExpr(NoPos, "x"), GtBop, ValExpr(NoPos, NumVal(2.0))),
    BopExpr(NoPos, CallExpr(NoPos, VarExpr(NoPos,"fib"), [BopExpr(NoPos, VarExpr(NoPos, "x"), MinusBop, ValExpr(NoPos,NumVal(1.0)))]), PlusBop, CallExpr(NoPos, VarExpr(NoPos,"fib"), [BopExpr(NoPos, VarExpr(NoPos, "x"), MinusBop, ValExpr(NoPos,NumVal(2.0)))])), 
    ValExpr(NoPos, NumVal(1.0)) )),
    None)))) 
let _ = Printf.printf "Task 4:\n%s\n" (str_program task4_program)
