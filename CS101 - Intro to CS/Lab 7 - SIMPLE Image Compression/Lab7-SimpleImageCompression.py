...
def isValid(password):
    # is it atleast 8 char long
    if (len(password) < 8):
        return False
    # if it doesn't have a number
    if not any(char in password) in string.digits:
        return False
    #if it doesn't have special character
    if not any(char in password) in string.punctuation:
        return False
    else:
        return True

# Driver Code
# prompt and get the user choice 1 or 2
