...
void Simulation::run() {
    // Attempt to make the given amount of free frames (as initially every frame free?)
    for (size_t i = 0; i < this->NUM_FRAMES; i++){
        free_frames.push_back(i);
    }

    // Provided code from here onwards.
...
char Simulation::perform_memory_access(const VirtualAddress& virtual_address) {
    ...
    if (this->flags.verbose) {
        ...
        std::cout << "\t -> RSS: " << this->processes[virtual_address.process_id]->get_rss() << std::endl;
    }
    return 0;
}
