...
# 52 "C:\Program Files (x86)\Microchip\xc8\v2.10\pic\include\pic18f26k22.h"
...
uint8_t redBufferIsEmpty = 1;
uint8_t blueBufferIsEmpty = 1;
uint8_t recording = 0;
...
printf("Sample Rate: %u us", 1600/16);
printMenu();
break;
...
blocksWritten = 0;
while(!(EUSART1_is_rx_ready()) && (blockIndex < 512)) {
for(i = 0; i < 512; i++) {
sdCardBuffer[i] = sin[sinIndex % 26];
sinIndex++;
}
...
if(i % 2 == 0) {
while(redBufferIsEmpty == 0);
SDCARD_ReadBlock(address, redBuffer);
redBufferIsEmpty = 0;
}
else {
while(blueBufferIsEmpty == 0);
SDCARD_ReadBlock(address, blueBuffer);
blueBufferIsEmpty = 0;
}
address += 512;
